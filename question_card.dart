...
class _QuestionCardState extends State<QuestionCard> {
  ...
  final TextEditingController _commentController = TextEditingController(); // للتحكم بنص التعليق

  // 💡 دالة إظهار نافذة تأكيد الحذف وتحديث عداد الاستفسارات في البروفايل
  void _showDeleteDialog(BuildContext context, String documentId, String ownerId) {
    showDialog(
      context: context,
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          ...
          title: const Text("حذف الاستفسار", style: TextStyle(color: Colors.white, fontFamily: 'Cairo')),
          content: const Text("هل أنت متأكد أنك تريد حذف هذا الاستفسار نهائياً؟", style: TextStyle(color: Colors.white70, fontFamily: 'Cairo')),
          actions: [
            TextButton(
            ...
            TextButton(
              child: const Text("حذف", style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.bold, fontFamily: 'Cairo')),
              onPressed: () async {
                try {
                  // 1. حذف مستند الاستفسار من كولكشن questions
                  await FirebaseFirestore.instance.collection('questions').doc(documentId).delete();
                  
                  // 2. إنقاص عدد استفسارات البروفايل بمقدار -1 لصاحب السؤال
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text("تم حذف الاستفسار بنجاح")),
                  );
                } catch (e) {
  ...
  void onReplyTap(dynamic widgetInstance, BuildContext context) {
    Map<String, dynamic> data = widgetInstance.document.data() as Map<String, dynamic>;
    String ownerId = data.containsKey('userId') ? data['userId'] : (data.containsKey('ownerId') ? data['ownerId'] : '');

    if (ownerId.isEmpty) {
      print("خطأ: لا يوجد معرف لصاحب المستند!");
      return;
    }
  }

  void _showCommentsBottomSheet(BuildContext context, String questionId, fs.DocumentSnapshot<Object?> document) {
    print("تم فتح قائمة التعليقات للاستفسار رقم: $questionId");
                  ...
                  Expanded(
                    child: StreamBuilder<QuerySnapshot>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        if (comments.isEmpty) {
                          return const Center(
                              child: Text("لا توجد ردود بعد", style: TextStyle(color: Colors.white54)));
                        }
                        ...
                        return ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            if (commentData.containsKey('timestamp') && commentData['timestamp'] != null) {
                              DateTime postDateTime = (commentData['timestamp'] as Timestamp).toDate();
                              commentTime = timeago.format(postDateTime, locale: 'ar');
                            }
                            ...
                            return FutureBuilder<DocumentSnapshot>(
                              future: FirebaseFirestore.instance.collection('users').doc(commenterUid).get(),
                              builder: (context, userSnapshot) {
                                ...
                                if (userSnapshot.hasData && userSnapshot.data!.exists) {
                                  ...
                                  realUsername = userData['username'] ?? 'مستخدم';
                                  realProfileImage = userData['profileImageUrl'];
                                }
                  ...
                  if (_replyingToUser != null)
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: const Border(right: BorderSide(color: Colors.orange, width: 4)),
                      ),
                      child: Row(
                        ...
                        children: [
                          Row(
                            children: [
                              const Icon(Icons.reply_rounded, color: Colors.orange, size: 18),
                              const SizedBox(width: 6),
                              Text("الرد على $_replyingToUser", style: const TextStyle(color: Colors.white, fontSize: 13)),
                            ],
                          ),
                          GestureDetector(
                            onTap: () {
                        ...
                        child: IconButton(
                          ...
                          onPressed: () async {
                            ...
                            try {
                              ...
                              String targetUserId = currentUser.uid;
                              String questionTitle = 'سؤالك';

                              if (questionDoc.exists && questionDoc.data() != null) {
                                final qData = questionDoc.data()!;
                                targetUserId = qData['userId'] ?? qData['uid'] ?? qData['user_id'] ?? currentUser.uid;
                                questionTitle = qData['text'] ?? 'سؤالك';
                              }

                              String realName = currentUser.displayName ?? 'مستخدم استفسر';
                              String realImage = currentUser.photoURL ?? '';

                              final userDoc = await FirebaseFirestore.instance.collection('users').doc(currentUser.uid).get();
                              if (userDoc.exists && userDoc.data() != null) {
                                final userData = userDoc.data()!;
                                realName = userData['name'] ?? userData['username'] ?? realName;
                                realImage = userData['image'] ?? userData['profileImageUrl'] ?? realImage;
                              }
                              ...
                              if (currentUser.uid != targetUserId) {
                                ...
                                print("🔥 نجاح تام: تم إرسال الإشعار لصاحب السؤال بنجاح!");
                              }
                            } catch (e) {
                              print("🚨 فشل في منطق الإشعارات ولكن تم تجاوز الخطأ: $e");
                            }
  ...
  // ميثود مخصصة لبناء عنصر التعليق السحب (ضع الكود الخاص بك بداخلها)
  Widget _buildSwipeableComment(String name, String text, String time, String replyName, String? img, VoidCallback onReply, StateSetter modalState) {
    return ListTile(
      leading: CircleAvatar(
        backgroundImage: img != null ? NetworkImage(img) : const NetworkImage('https://via.placeholder.com/150'),
      ),
      title: Text(name, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
      subtitle: Text(text, style: const TextStyle(color: Colors.white70)),
      trailing: IconButton(
        icon: const Icon(Icons.reply, color: Colors.orange, size: 20),
        onPressed: onReply,
      ),
    );
  }
 ...
 Future<void> sendNotificationOnReply({
  ...
  try {
    ...
    if (questionDoc.exists && questionDoc.data() != null) {
      ...
      // 📌 [تعديل مهم]: جلب الـ ID الخاص بصاحب السؤال بشكل صحيح ومضمون
      final targetUserId = qData['userId'] ?? qData['uid'] ?? qData['ownerId'];
      final questionTitle = qData['title'] ?? 'سؤالك';

      // التأكد من أن الـ ID تم العثور عليه، وأنه ليس نفس الشخص الذي قام بالرد
      if (targetUserId != null && targetUserId != currentUserId) {
        // 3. كتابة الإشعار في الفايربيس
        await FirebaseFirestore.instance.collection('notifications').add({
          'to': targetUserId, // الـ UID الخاص بصاحب السؤال الموجه له الإشعار
